...
| `FigureNode` | 组合 NodeState、LayoutState 与具体 Figure | 不持有平台或全局服务 |
| `FigureTree` | 维护 SlotMap、树关系和 Z-order | 不持有交互 session 或更新队列 |
| `InteractionState` | 维护 pointer、focus、capture、hover 和 gesture | 只通过 FigureId 引用节点 |
| `Runtime` | 协调事件、effect、mutation 和更新事务 | 不暴露可绕过事务的多个可变引用 |
| `UpdateManager` | 编排 Validation 和 Damage Repair | 不承担平台调度 |
| `EventDispatcher` | 选择 target 并推进输入状态机 | 平台无关，不拥有 FigureTree |

### 变更规则与工程约束

- **延迟应用 (Pending Mutation)**: 所有的结构性变更（如 `add_child`）必须先进入
  MutationQueue。在事件分发的回调栈中，禁止直接修改 FigureTree，以防破坏遍历一致性。
